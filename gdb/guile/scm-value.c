...
gdbscm_make_value (SCM x, SCM rest)
{
  ...
  value = vlscm_convert_value_from_scheme (x, type, type_scm,
					   SCM_ARG1, FUNC_NAME,
...
gdbscm_value_subscript (SCM self, SCM index_scm)
{
  ...
  index = vlscm_convert_value_from_scheme (index_scm, NULL, SCM_UNDEFINED,
...
gdbscm_value_call (SCM self, SCM args)
{
  ...
  SCM_ASSERT_TYPE (TYPE_CODE (ftype) == TYPE_CODE_FUNC, self,
		   SCM_ARG1, FUNC_NAME, "function (value of TYPE_CODE_FUNC)");

  SCM_ASSERT_TYPE (gdbscm_is_true (scm_list_p (args)), args,
		   SCM_ARG2, FUNC_NAME, "list");
  ...
  if (args_count > 0)
      ...
      for (i = 0; i < args_count; i++)
	{
	  ...
	  vargs[i] = vlscm_convert_value_from_scheme (arg, NULL, SCM_UNDEFINED,
						      0, FUNC_NAME,
						      &except_scm,
						      gdbarch, language);
	  if (vargs[i] == NULL)
...
gdbscm_value_to_real (SCM self)
{
  ...
  SCM_ASSERT_TYPE (is_intlike (type, 0) || TYPE_CODE (type) == TYPE_CODE_FLT,
		   self, SCM_ARG1, FUNC_NAME, "number");
  ...
  if (d != (double) d)
    return gdbscm_out_of_range_error (FUNC_NAME, SCM_ARG1, self,
...
\n\
  ...
  { "value-call", 2, 0, 0, gdbscm_value_call,
    "\
Make an inferior function call taking the value as a pointer to the\n\
function to call.\n\
