...
gdbscm_set_ignore_count_x (SCM self, SCM newvalue)
{
  ...
  SCM_ASSERT_TYPE (scm_is_signed_integer (newvalue, LONG_MIN, LONG_MAX),
		   newvalue, SCM_ARG2, FUNC_NAME, "integer");
...
gdbscm_set_thread_x (SCM self, SCM newvalue)
{
  ...
  if (scm_is_signed_integer (newvalue, LONG_MIN, LONG_MAX))
    ...
    SCM_ASSERT_TYPE (0, newvalue, SCM_ARG2, FUNC_NAME, "integer or #f");
...
gdbscm_breakpoint_cond_says_stop (struct breakpoint *b)
{
  ...
  gdb_assert (gdbscm_is_procedure (bp_smob->stop));
  stop = 1;
