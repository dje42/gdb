...
/* All gdb smobs contain either gdb_smob or chained_gdb_smob as the first
   member.
   ...
   IMPORTANT: chained_gdb_smob is a "subclass" of gdb_smob.  The layout
   of chained_gdb_smob must match gdb_smob as if it is a subclass.
   To that end we use macro GDB_SMOB_HEAD to ensure this.  */
...
extern SCM gdbscm_make_arg_error (SCM key, const char *subr, int arg_pos,
...
extern SCM bkscm_gsmob_from_block (const struct block *block,
...
extern SCM lsscm_safe_call_lazy_string_to_value (SCM self);
...
extern SCM stscm_gsmob_from_symtab (struct symtab *symtab);

extern SCM stscm_scm_from_symtab_unsafe (struct symtab *symtab);

extern SCM stscm_gsmob_from_sal (struct symtab_and_line sal);
...
extern struct value *vlscm_convert_value_from_scheme
  (SCM obj, struct type *type, SCM type_scm,
   int arg_pos, const char *func_name, SCM *except_scmp,
...
extern SCM tyscm_gsmob_from_type (struct type *type);
...
extern int gdbscm_auto_load_enabled (void);
