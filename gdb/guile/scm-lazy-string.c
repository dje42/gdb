...
typedef struct {
  ...
  /*  Holds the encoding that will be applied to the string when the string
      is printed by GDB.  If the encoding is set to #f then GDB will select
...
/* Struct used to pass data from lsscm_safe_call_lazy_string_to_value to
   gdbscm_call_lazy_string_to_value.  */

struct lsscm_lazy_string_to_value_data
{
  SCM obj;
};

/* Helper for lsscm_safe_call_lazy_string_to_value to call
   gdbscm_lazy_string_to_value.  */

static SCM
lsscm_call_lazy_string_to_value (void *datap)
{
  struct lsscm_lazy_string_to_value_data *data = datap;

  return gdbscm_lazy_string_to_value (data->obj);
}

/* A "safe" version of gdbscm_lazy_string_to_value for use by
   vlscm_convert_value_from_scheme.
   If an exception is thrown in the call, a <gdb:exception> object
   is returned.  */

SCM
lsscm_safe_call_lazy_string_to_value (SCM self)
{
  struct lsscm_lazy_string_to_value_data data;
  SCM result;

  data.obj = self;
  result = gdbscm_call_guile (lsscm_call_lazy_string_to_value, &data, NULL);
