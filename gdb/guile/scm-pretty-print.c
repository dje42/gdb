...
gdbscm_make_pretty_printer (SCM name, SCM lookup)
{
  ...
  SCM_ASSERT_TYPE (scm_is_string (name), name, SCM_ARG1, FUNC_NAME, "string");
  ...
  *out_value = NULL;
  TRY_CATCH (except, RETURN_MASK_ALL)
      ...
      else if (vlscm_is_value (v_scm = vlscm_scm_to_value_gsmob (result)))
	{
	  ...
	  *out_value =
	    vlscm_convert_value_from_scheme (v_scm, NULL, SCM_UNDEFINED,
					     GDBSCM_ARG_NONE, FUNC_NAME,
		      ...
		      int printed_nothing)
{
  ...
  for (i = 0; i < options->print_max; ++i)
      ...
      else if (! is_map)
	{
      ...
      ls_scm = lsscm_scm_to_lazy_string_gsmob (scm_v);
      ...
      else if (gdbscm_is_exception (ls_scm))
	{
      ...
      else if (scm_is_string (scm_v))
	{
	  char *output = gdbscm_scm_to_c_string (scm_v);
      ...
      else
	{
	  SCM except_scm;
	  struct value *value =
	    vlscm_convert_value_from_scheme (scm_v, NULL, SCM_UNDEFINED,
					     GDBSCM_ARG_NONE, FUNC_NAME,
					     &except_scm, gdbarch, language);
