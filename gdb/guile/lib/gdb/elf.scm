...
(set-record-type-printer! <elf>
  (lambda (elf port)
    (format port "#<elf bytes: vu8(...) word-size: ~a byte-order: ~a abi: ~a type: ~a machine-type: ~a entry: ~a phoff: ~a shoff: ~a flags: ~a ehsize: ~a phentsize: ~a phnum: ~a shentsize: ~a shnum: ~a shstrndx: ~a"
            (elf-word-size elf) (elf-byte-order elf) (elf-abi elf) (elf-type elf) (elf-machine-type elf) (elf-entry elf)
            (elf-phoff elf) (elf-shoff elf) (elf-flags elf) (elf-ehsize elf) (elf-phentsize elf) (elf-phnum elf) 
            (elf-shentsize elf) (elf-shnum elf) (elf-shstrndx elf))))
