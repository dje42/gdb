...
/* A smob describing a gdb block.
   A block object is associated with an objfile, so use a chained_gdb_smob
   to keep track of all blocks associated with the objfile.  This lets us
   invalidate the underlying struct block when the objfile is deleted.  */
...
static SCM block_syms_iter_next_x_proc;
...
bkscm_mark_block_smob (SCM self)
{
  ...
  /* Do this last.  */
  return gdbscm_mark_chained_gsmob (&b_smob->base);
}
...
bkscm_free_block_smob (SCM self)
{
  ...
  gdbscm_remove_objfile_ref (b_smob->objfile, bkscm_objfile_data_key,
			     &b_smob->base);
...
/* The smob "equalp" function for <gdb:block>.  */

static SCM
bkscm_equal_p_block_smob (SCM b1, SCM b2)
{
  const block_smob *b1_smob = (block_smob *) SCM_SMOB_DATA (b1);
  const struct block *b1_block = b1_smob->block;
  const block_smob *b2_smob = (block_smob *) SCM_SMOB_DATA (b2);
  const struct block *b2_block = b2_smob->block;

  return scm_from_bool (b1_block == b2_block);
}
...
bkscm_make_block_smob (void)
{
  ...
  b_smob->containing_scm = b_scm;
  gdbscm_init_chained_gsmob (&b_smob->base);
...
bkscm_gsmob_from_block (const struct block *block, struct objfile *objfile)
{
  ...
  gdbscm_add_objfile_ref (objfile, bkscm_objfile_data_key, &b_smob->base);
  b_smob->block = block;
...
/* Returns a new <gdb:block> object that encapsulates BLOCK.
   Throws a Scheme exception if an error occurs during the conversion.  */

SCM
bkscm_scm_from_block_unsafe (const struct block *block,
			     struct objfile *objfile)
{
  SCM smob = bkscm_gsmob_from_block (block, objfile);

  return gdbscm_scm_from_gsmob_unsafe (smob);
}
...
bkscm_get_valid_block (SCM scm, int arg_pos, const char *func_name, SCM *excp)
{
  ...
  if (!bkscm_is_valid (b_smob))
    {
      *excp = gdbscm_make_arg_error (gdbscm_invalid_object_error_symbol,
				     func_name, arg_pos, scm,
				     _("Invalid block"));
      return NULL;
    }
...
{
  block_smob *b_smob = datum;

  while (b_smob != NULL)
    {
      block_smob *next = (block_smob *) b_smob->base.next;

      b_smob->block = NULL;
      b_smob->objfile = NULL;
      b_smob->base.next = NULL;
      b_smob->base.prev = NULL;
...
gdbscm_make_block_syms_iter (SCM self)
{
  ...
  iter = gdbscm_make_iterator (self, progress, block_syms_iter_next_x_proc);
...
static SCM
gdbscm_block_syms_iter_next_x (SCM self)
{
...
gdbscm_initialize_blocks (void)
{
  ...
  scm_set_smob_equalp (block_smob_tag, bkscm_equal_p_block_smob);
  ...
  block_syms_iter_next_x_proc =
    scm_c_define_gsubr ("%block-syms-iter-next!", 1, 0, 0,
			gdbscm_block_syms_iter_next_x);
